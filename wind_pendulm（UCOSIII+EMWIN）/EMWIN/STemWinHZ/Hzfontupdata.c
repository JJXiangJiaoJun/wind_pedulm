 	...
 	if(rval==0)	 
	{
		...
		while(res==FR_OK)//死循环执行
		{
			...
			W25QXX_Write(tempbuf,offx+flashaddr,4096);		    //从0开始写入4096个数据  
	  		offx+=bread;	  
			emwin_fupd_prog(fftemp->obj.objsize,offx);	    //进度显示
...
{	
	...
	{  
		...
		{
			emwin_fupd_prog(FONTSECSIZE,i);//进度显示
		...
		{
            TEXT_SetText(hItem,(const char*)UPDATE_REMIND_TBL[i]);  //修改TEXT1控件内容
            GUI_Exec();
			strcpy((char*)pname,(char*)src);				//copy src内容到pname
			strcat((char*)pname,(char*)GBK_PATH[i]); 		//追加具体文件路径 
...
void HzFontupdataCreate(void) 
{
    ...
    while(1)
    {
        ...
        emwin_update_font("0:");        //更新字库 
        OSSchedUnlock(&err);	        //调度器解锁
        WM_DeleteWindow(HzfontWin);     //删除窗口
        GUI_Delay(100);
        break;                          //更新完成，跳出循环
    }
}
